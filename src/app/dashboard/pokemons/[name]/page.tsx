...
const getPokemons = async (limit = 20, offset = 0): Promise<SimplePokemon[]> => {
  ...
  const data: PokemonsResponse = await fetch(`https://pokeapi.co/api/v2/pokemon?limit=${limit}&offset=${offset}`).then(resp => resp.json())
...
export async function generateStaticParams() {
  ...
  const pokemonStatic = await getPokemons(151);

  return pokemonStatic.map((name) => ({
    name: name.name
  }))
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    ...
    return {
      title: 'Pokemon',
      description: 'Lorem ipsum dolor sit amet consectetur, adipisicing elit. Laboriosam, corrupti.'
    }
  }
